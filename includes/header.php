...
<head>
    ...
    <style>
        ...
        .nav-link-active {
            color: var(--tw-colors-primary); /* Active nav item with primary blue */
            font-weight: 600;
            background-color: var(--tw-colors-neutral-focus); /* Subtle background for active link */
        }
        ...
        pre {
            ...
            padding: 10px 15px; /* Added from your snippet */
            margin-bottom: 15px; /* Added from your snippet */
            overflow-x: auto; /* Added from your snippet */
            border-radius: 4px; /* Added from your snippet for box-shadow consistency */
        }
        code {
            font-family: 'Courier New', Courier, monospace;
            color: var(--tw-colors-accent); /* Reddish accent for code */
            white-space: pre-wrap; /* Added from your snippet */
        }
        strong {
        ...
        .bg-secondary.hover\:bg-secondary-hover.text-white:hover {
            background-color: var(--tw-colors-primary-hover); /* Darker primary blue */
        }


        /* Main Navigation (Header) */
        .bg-base-200\/80 { /* Header background */
            background-color: rgba(var(--tw-colors-neutral-rgb, 255 255 255) / 0.9); /* Slightly more opaque white */
        ...
        .hover\:bg-neutral-focus.hover\:text-secondary:hover { /* Nav link hover text (targeting specific compound class) */
            color: var(--tw-colors-primary); /* Use primary blue for main nav link hover */
        }
        .text-secondary { /* Site logo icon in header (now primary) */
            color: var(--tw-colors-primary);
        }
        .text-secondary:hover { /* Site logo icon hover (now primary-hover) */
            color: var(--tw-colors-primary-hover);
        }
        /* Admin Panel link in main nav */
        a[href$="admin/"][class*="text-slate-300"] {
            color: var(--tw-colors-neutral-content);
        }
        a[href$="admin/"][class*="hover\:text-secondary"]:hover {
            color: var(--tw-colors-primary);
        }
        /* Sign Out button in main nav */
        .bg-red-600.hover\:bg-red-700.text-white {
            background-color: var(--tw-colors-accent); /* Using red accent for logout */
            color: #fff;
        }
        .bg-red-600.hover\:bg-red-700.text-white:hover {
            background-color: var(--tw-colors-accent-hover);
        }
        ...
        .mobile-menu .hover\:text-secondary:hover { /* Mobile menu links hover */
            color: var(--tw-colors-primary);
        }
        .mobile-menu .text-secondary.bg-neutral-focus { /* Mobile active link */
            color: var(--tw-colors-primary); /* Primary blue */
            background-color: var(--tw-colors-neutral-focus); /* Light gray background */
        }


        /* Global text colors often set by Tailwind utility classes applied directly in HTML */
        .text-slate-400 { /* General gray text in various components */
            color: var(--tw-colors-text-strong); /* Softer dark gray */
        }
        .text-slate-300 { /* Lighter gray text in various components */
            color: var(--tw-colors-neutral-content); /* Main dark text */
        }
        .text-secondary { /* Elements explicitly using text-secondary */
            color: var(--tw-colors-primary); /* Use primary blue for them */
        }
        .text-secondary:hover { /* Hover for elements using text-secondary */
            color: var(--tw-colors-primary-hover);
        }
...
<body class="antialiased bg-base-100 text-neutral-content flex flex-col min-h-screen">
    ...
    <div class="bg-neutral text-slate-400 py-2 px-4 sm:px-6 lg:px-8 border-b border-neutral-lighter text-xs print:hidden">
    ...
    <header id="mainNav" class="bg-base-200/90 backdrop-blur-lg shadow-lg sticky top-0 z-50 border-b border-neutral-lighter print:hidden">
            ...
            <div class="flex justify-between items-center h-16">
                ...
                <nav class="hidden md:flex items-center space-x-1">
                    ...
                    <?php if (isset($_SESSION['admin_user_id'])): ?>
                        ...
                        <a href="<?php echo BASE_URL; ?>admin/index.php?admin_page=logout" class="ml-2 px-3 py-1.5 rounded-md text-sm font-medium bg-accent hover:bg-accent-hover text-white transition-colors flex items-center space-x-1"><i data-lucide="log-out" class="w-4 h-4 mr-1"></i>Sign Out</a>
                    <?php endif; ?>
                </nav>
        ...
        <div id="mobileMenu" class="mobile-menu md:hidden bg-neutral border-t border-neutral-lighter absolute w-full shadow-xl left-0">
            <nav class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
                <?php foreach($main_nav_links as $link_item): ?>
                     ...
                     <a href="<?php echo $link_url_mobile; ?>" class="block px-3 py-3 rounded-md text-base font-medium <?php echo ($current_public_page === $link_item['page'] ? 'nav-link-active' : 'text-neutral-content hover:bg-neutral-focus hover:text-primary'); ?> transition-colors flex items-center space-x-2">
                ...
                <div class="border-t border-neutral-lighter pt-4 mt-4 space-y-2">
                    <?php if (isset($_SESSION['admin_user_id'])): ?>
                        ...
                        <a href="<?php echo BASE_URL; ?>admin/index.php?admin_page=logout" class="block w-full text-left px-3 py-3 rounded-md text-base font-medium bg-accent hover:bg-accent-hover text-white transition-colors">Sign Out</a>
                    <?php else: ?>
