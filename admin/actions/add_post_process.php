...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['submit_post'])) {
    ...
    if (!empty($slug)) { 
        ...
        if(!$stmt_slug_check) { $errors[] = "DB error preparing slug check: " . $conn->error; }
            ...
            if ($stmt_slug_check->num_rows > 0) {
                $errors[] = "This slug ('" . esc_html($slug) . "') is already in use.";
            }
    ...
    if (isset($_FILES['featured_image']) && $_FILES['featured_image']['error'] === UPLOAD_ERR_OK) {
        ...
        $errors[] = "Error uploading featured image. Code: " . $_FILES['featured_image']['error'];
    }
    ...
    $featured_image_to_save = empty($featured_image_filename) ? null : $featured_image_filename;
    ...
    $sql = "INSERT INTO posts (title, slug, content, category_id, status, featured_image, author_id, meta_description, meta_keywords, excerpt, created_at, updated_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())";
    $stmt = $conn->prepare($sql);
    if ($stmt) {
        // Types: title(s), slug(s), content(s), category_id(i), status(s), 
        // featured_image(s), author_id(i), meta_desc(s), meta_key(s), excerpt(s)
        $stmt->bind_param("sssiisssss", 
            $title, $slug, $clean_content, $category_id_to_save, $status_to_save, 
            $featured_image_to_save, $author_id, 
            $meta_description_to_save, $meta_keywords_to_save, $excerpt_to_save
        );
        
        if ($stmt->execute()) {
            $_SESSION['flash_message'] = "Post created successfully with status: '" . esc_html($status_to_save) . "'";
            $_SESSION['flash_message_type'] = "success";
            header('Location: ' . $admin_base_url . 'index.php?admin_page=posts');
            exit;
        } else {
            error_log("DB Error adding post: (" . $stmt->errno . ") " . $stmt->error);
            $_SESSION['flash_message'] = "Error creating post. DB Code: " . $stmt->errno . " - " . esc_html($stmt->error);
            $_SESSION['flash_message_type'] = "error";
        }
        $stmt->close();
    } else {
        error_log("DB Prepare Error adding post: " . $conn->error);
        $_SESSION['flash_message'] = "Database error preparing statement for post creation: " . esc_html($conn->error);
        $_SESSION['flash_message_type'] = "error";
    }
